        ...
        with open(propertyFileName) as propertyFile:
            for line in propertyFile:
                ...
                if infoStringYaml["stateArg"] == -1:
                    nodeLabel = (infoStringYaml["node"], infoStringYaml["sender"], infoStringYaml["state"])
                else:
                    nodeLabel = (infoStringYaml["node"], infoStringYaml["sender"], infoStringYaml["state"])
...
#                         for node in self.nodes}
        ...
        traces = []
        allStates = [(actorNode, sender, state) for actorNode in range(self.numMachines) for sender in range(self.numMachines) for state in ["looking", "committed", "end"]]
